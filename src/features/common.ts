 ...
 * @param variantColors - The variant colors data
 * @param customOrder - Optional custom property order (if not provided, uses default)
 */
function getVariantTitle(
  ...
  // Normalize priorityOrder to lowercase for comparison
 ...
 * @param propertyMap - The property map to format
 * @param customOrder - Optional custom property order (if not provided, uses default)
 */
export function formatVariantPropertiesForTable(
  ...
  for (const prop of normalizedPriorityOrder) {
    if (propertyMap[prop]) {
      // Only push the value, without the label prefix
      parts.push(propertyMap[prop]);
    }
  }

  // Only add remaining properties if NO custom order was provided
  // If customOrder exists, only show the properties explicitly listed
  if (!customOrder) {
    for (const [key, value] of Object.entries(propertyMap)) {
      if (!normalizedPriorityOrder.includes(key) && value) {
        // Only push the value, without the label prefix
...
export async function createGenericVariantGrid(
  ...
  // Se há apenas uma variante, usar largura total; caso contrário, dividir por colunas
