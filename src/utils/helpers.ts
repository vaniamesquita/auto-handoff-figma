...
export function resolveSpacingElement(node: SceneNode): string {
  ...
  // Return the node's own name when it's meaningful
  return node.name;
}
...
/**
 * Formats a dimension (height/width) variable name into a token string.
 * Strips Figma category prefixes (shapes/).
 */
export function formatDimensionToken(name: string): string {
...
/**
 * Formats a border radius variable name into a token string.
 * Strips Figma category prefixes (border-radius/, shapes/).
 */
export function formatRadiusToken(name: string): string {
  let cleaned = name.toLowerCase();
  cleaned = cleaned.replace(/\//g, "-").replace(/\s+/g, "-");
