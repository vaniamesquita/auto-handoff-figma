...
import type { BoundVariablesRecord, VariableAliasBinding } from "../types";
...
/**
 * Determines if an icon instance should be collected, based on whether
 * there is a non-structural instance barrier between it and the component root.
 * Structural instances and frames/groups are transparent (not barriers).
 *
 * @param iconNode - The icon instance (INSTANCE with direct Vector child)
 * @param componentRoot - The top-level component node being analysed
 * @returns True if the icon belongs to the component (no barriers in path)
 */
export async function shouldCollectIcon(
  iconNode: SceneNode,
  componentRoot: SceneNode,
): Promise<boolean> {
  let current: BaseNode | null = iconNode.parent;
  while (current !== null && current !== componentRoot) {
    if (current.type === "INSTANCE") {
      const isStructural = await isStructuralInstance(current as InstanceNode);
...
export async function findTextNodes(
  ...
  skipNonStructural: boolean = false,
): Promise<TextNode[]> {
  ...
  // If nested instance, check if structural (only when skipNonStructural is true)
...
export async function resolveBoundVariable(
  ...
  const boundVars = node.boundVariables as BoundVariablesRecord;
  const binding = boundVars[property] as VariableAliasBinding | undefined;
...
export async function resolveBoundVariableAtIndex(
  ...
  const binding = bindings[index] as VariableAliasBinding | undefined;
  if (binding?.type === "VARIABLE_ALIAS") {
    const variable = await figma.variables.getVariableByIdAsync(binding.id);
    if (variable) {
      return formatter ? formatter(variable.name) : variable.name;
