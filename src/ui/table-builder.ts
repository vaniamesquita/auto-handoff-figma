...
import { TEXT_COLORS } from "../config/theme";
...
export class TableBuilder {
  ...
  public addRow(rowName: string, cells: (string | CellData)[]) {
    ...
    row.resize(this.width, 1); // Altura mínima, será expandida pelo conteúdo
    row.fills = [{type: "SOLID", color: {r: 1, g: 1, b: 1}}];
    row.cornerRadius = 4;
    row.itemSpacing = 0;
    row.counterAxisAlignItems = "MIN"; // Alinha ao topo
    row.paddingTop = 12;
    row.paddingBottom = 12;

    // IMPORTANTE: Define sizing APÓS adicionar ao parent com auto-layout
    // Por enquanto, configura as células primeiro
    ...
    this.columns.forEach((colConfig, index) => {
      ...
      cellFrame.primaryAxisSizingMode = "AUTO"; // Hug height
      cellFrame.counterAxisSizingMode = "FIXED"; // Fixed width
      cellFrame.fills = [];

      // Calcula a largura baseada na porcentagem da coluna
      const colWidth = this.getColumnWidth(index);
      cellFrame.resize(colWidth, 1); // Altura mínima
      ...
      // Configura o texto para quebrar linha e crescer verticalmente
      textNode.textAutoResize = "HEIGHT";
      textNode.layoutAlign = "STRETCH";

      cellFrame.appendChild(textNode);

      // Após adicionar o texto, força a célula a usar HUG no eixo vertical
    ...
    // Após adicionar ao container, força a row a usar HUG no eixo vertical
